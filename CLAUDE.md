...
> Last updated: 6 May 2026 (session 37 — silent Madrid scraper-defaults bug, results not settling). See "Session-end protocol" at the bottom of this file — follow it at the end of every session.

---

## Working agreement (READ FIRST)

Before touching any code in a Cowork session, read
[`docs/working-agreement.md`](docs/working-agreement.md). It is the
binding rule on how Claude ships changes to this live, user-facing
product.

**Summary of the rule:** no direct pushes to `main` for user-facing
changes. Every such change goes through a feature branch, a Vercel
preview URL, smoke against the preview, a PR, CI green, Mickey's
explicit approval, then merge.

Documented exceptions cover:
- Prod-down emergencies (with explicit per-incident sign-off, plus
  a follow-up regression test within 24h)
- Memory / docs / CI-workflow / admin-endpoint changes (these can
  go direct)

Why this exists: 6 May 2026 we shipped 4 user-facing bugs to prod
in a single session because changes went straight to `main` while
real users were trying to play. The discipline below is the fix.
...
| 6 May 2026 (session 37) | **Critical — scraper silently scraped Madrid for a week.** Mickey reported R1 results not settling on bracket/leaderboard. Diagnosis: bracket returned `seed_draw+scraper(0)` — every fixture rejected by the overlay. Trace: scraper cache had a *completed Final* (Sinner d. Zverev 6-1, 6-2 — Madrid result, not possible for Rome which started today) plus completed SF/QF/R16/R32/R64 with synthetic 06 May timestamps. Scraper was scraping Madrid's archive page. **Root cause:** Railway scraper service `valiant-forgiveness` (id `012860d6-07a0-48f1-8818-ccc4625188a0`) had only 3 env vars set (ADMIN_SECRET, BACKEND_URL, DEFAULT_ROUND). `FLASHSCORE_URL` and `RESULTS_URL` were *missing entirely*. `scraper/src/config.mjs` had Madrid URLs hardcoded as defaults, so the scraper silently pulled Madrid for the entire week between Madrid completion (29 Apr) and Rome launch (06 May). CLAUDE.md previously claimed "Scraper env vars updated for Rome" but they were never actually set. **Fix sequence:** (1) Took computer control, navigated to Railway scraper service → Variables → Raw Editor, added FLASHSCORE_URL=`https://www.flashscore.co.uk/tennis/atp-singles/rome/`, RESULTS_URL=`https://www.flashscore.co.uk/tennis/atp-singles/rome/results/`, TIMEZONE_OFFSET=2. (2) Deploy + cron Run now. First fresh scrape at 17:14 UTC: 32 R1 fixtures (13 completed, 19 scheduled — matches today's mid-afternoon state). (3) Bracket immediately recovered: dataSource went from `seed_draw+scraper(0)` to `seed_draw+scraper(31)`. Confirmed Brooksby vs Baez → Baez S. won (matches Mickey's claim — Mickey was right; my earlier "Gaubas won" claim was the Madrid data leaking through). Leaderboard now shows correct alive-status per pick. **PR #4** (`fix/scraper-no-madrid-defaults`) opened: `requireEnv()` helper in `scraper/src/config.mjs` throws on missing/blank `FLASHSCORE_URL`/`RESULTS_URL` instead of falling back to Madrid. Next missed env var triggers a loud Railway crash, not silent wrong data. **Doc hardening:** `docs/transition-prompt.md` + `docs/paid-transition-prompt.md` now have a CRITICAL block on the scraper env var section with explicit verification commands (R1 fixtures count + bracket dataSource); cannot move to Phase 6 without proving both pass. URLs corrected from `.com` to `.co.uk` (matching production). `scripts/smoke.sh` extended with a 1b step that flags `seed_draw+scraper(0)` post-launch as a regression. `scripts/validate-tournament.mjs` ends with a Railway env-var reminder so the static check doesn't lull anyone into a false sense of safety. New memory file `feedback_silent_scraper_defaults.md`. **Lesson:** when CLAUDE.md says "X was updated" but no commit/screenshot/log proves it, treat it as unverified. Same class as the 17 Apr stale-mnt incident — claims of completed work that weren't actually completed. |
| 6 May 2026 (session 36) | **Rome R1 launch incident: phantom picks + frontend deadline buffer.** Real users (Mickey's friends) reported they could not join Rome despite seeing successful UI states. Three bugs found and fixed: (1) Frontend GroupHome.jsx computed entryDeadline = r1LockAt - 1h — buffer intended for late joiners but instead hid Join button before backend closed entries (commit 86441b1). (2) Backend R1 lock pushed from 06 May 08:00 UTC to 09:00 UTC (d2461ed). (3) Critical data-integrity bug: picks.js only rejected picks if the (user, group) row existed AND is_alive=false. If user was not a member at all, the pick was inserted anyway. Symptom: user saw 'Current R1 Pick' on screen but wasn't in leaderboard. One real orphan pick (Mick / pickssmicks@gmail.com) found and recovered (b65bd76). Recovery + prevention: f78049d /admin/recent-users; a919583 /admin/orphan-picks + /admin/picks-by-player; e35e3aa /admin/bulk-add-members; scripts/smoke.sh extended with orphan-pick check + non-member pick rejection regression; docs/transition-prompt.md + paid-transition-prompt.md Phase 8 fully rewritten (multi-account incognito test, API verification after each UI action, orphan-pick check, frontend-vs-backend deadline alignment); feedback_06may_phantom_picks.md documents the lesson. Final Rome lineup: Rafa, Casper, Mick, Mark. **Workflow change adopted (`9944de3` and follow-ups):** all user-facing changes now route through feature branches + Vercel preview + PR + Mickey approval. Documented in `docs/working-agreement.md`. Direct pushes to `main` reserved for memory/docs/CI/admin-only changes and explicit prod-down emergencies. **First PR cycle exercised end of session:** PR #1 (`claude/seo-basics`) added robots.txt, sitemap.xml, canonical URL, JSON-LD with brand-misspell aliases ("Final Serveivor", "Final Survivor Tennis", "FinalServeivor"), theme-color meta. Merged as `c5fe659`. Google Search Console domain property for `finalserveivor.com` verified via TXT record (`google-site-verification=28bF0UID_eealiUCa756rQ6s9beWCFJ4xCPDIXHYX3E`). Sitemap submitted; status "Couldn't fetch" pending Google retry (Vercel apex→www redirect not followed on first attempt). Lesson: don't transcribe precise strings from screenshots — pull from DOM. Captured in `feedback_screenshot_truncation.md`. SEO state in `project_seo_infrastructure.md` with canonical/www follow-ups documented. **Second PR (`daa8054`, PR #2):** draw display fixes. `shortName()` was inverting every player's name because seed-draw stores `"Surname, Firstname"` and the function assumed `"Firstname Lastname"` — every bracket card and matchup modal showed `"Jannik, S."` instead of `"Sinner, J."`. Fix handles canonical comma format, multi-name first names (`"Cerundolo, J.M."`), multi-word surnames (`"Carreno Busta, P."`), hyphenated names (`"Struff, J.-L."`), and a placeholder guard for `"Qualifier 13"`. 8 Node-test cases at `frontend/tests/playerImage.test.mjs` runnable via `npm test`. Player avatar face offset bumped 10%→18% for better face centering across the 20–56px size range. **CORS fix bundled (`b4b4481`, direct push):** `ALLOWED_ORIGINS` was static and didn't include Vercel preview URLs, so login on the preview returned 'Failed to fetch'. Replaced with a function + regex pattern locked to the project's Vercel team subdomain. Pre-requisite for the working-agreement preview-verify step to actually function. **Smoke step 6 made tolerant** of post-lock state — pick endpoint returns 400 (round locked) once a round closes, before the membership check fires; both 400 and 403 are non-regressions. **Third PR (`3f9f7fc`, PR #3):** non-picker elimination architectural fix. Mark didn't pick R1, leaderboard still showed him alive at 09:35 UTC because `eliminateNonPickers()` was only a sub-step of `processRoundResults()`, which fires when matches complete — for first round, that's a 1-2 hour window of incorrect leaderboard. Added `sweepLockedRoundNonPickers()` walked from 15-min cron, eliminates non-pickers as soon as a round locks. Mark eliminated manually via `/api/admin/eliminate-non-pickers` for immediate fix (visible on leaderboard). New lesson at `feedback_non_picker_elimination_timing.md`. |
...
### Before pushing
0. **Confirm `docs/working-agreement.md` route applies.** User-facing changes go to a feature branch, not `main`. See exception list in that file.
