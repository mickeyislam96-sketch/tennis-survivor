...
> Last updated: 7 May 2026 (session 36). See "Session-end protocol" at the bottom of this file — follow it at the end of every session.
...
13. **Wire UptimeRobot to /api/health** — backend ready (returns 503 with `scraper_freshness: STALE`), needs Mickey's UptimeRobot account + 5-min monitor + SMS alert. Critical-gap #1.
14. **Stage 2 admin token rollout** — implement `ADMIN_TOKEN_FINANCIAL` and gate refunds/payouts/settlement before RG (18 May). Critical-gap #4 stage 2.
15. **Verify pg_dump restore** — daily backup runs but restore never tested. Critical-gap #5. Quarterly cadence after first verification.
...
17. **Add `staging` branch trigger to GitHub Actions tests.yml** — currently CI only runs on main, defeating the purpose of staging.
...
| 7 May 2026 (session 36) | **Staging environment + AI agent ops layer.** Major infra + tooling lift. (1) **Staging environment shipped end-to-end.** Created `staging` branch in GitHub from main's SHA `9ee8853`. In Railway, duplicated the production environment in the existing project — new env ID `6e2a12c6-df61-45dc-89e0-d8e71ca0d14f` with isolated Postgres, separate backend service URL `tennis-survivor-staging.up.railway.app`, separate scraper. Switched the staging backend service to deploy from `staging` branch. Replaced the literal copied `DATABASE_URL` with a `${{Postgres.DATABASE_URL}}` reference so it auto-resolves to the staging Postgres per environment. (2) **Vercel staging preview wired.** Added a Preview-scoped `VITE_API_URL` env var in Vercel scoped to the `staging` branch, pointing at the staging Railway URL — production frontend remains untouched. Pushed `STAGING.md` to the staging branch to trigger Vercel's first preview build (status 201). Predicted preview URL: `tennis-survivor-git-staging-mickeyislam96-sketchs-projects.vercel.app`. (3) **Smoke verified.** `/api/health` returned 200 on both prod (scraper data, db ok) and staging (no scraper data yet, db ok, tournament=rome-2026). `/api/draw/rounds` returned the Rome round list from staging. (4) **Deleted orphan `pleasing-appreciation` Railway project** (project ID `129e23d9-96bb-41ee-960b-48fbaac7378a`) — duplicate connection to the same GitHub repo with a misconfigured tennis-survivor service that had been failing every build for 12+ hours, generating ~10 emails/day. 48-hour grace period on the deletion. (5) **Built `fsv-daily-brief` skill.** Lives at `.claude/skills/fsv-daily-brief/SKILL.md` in the repo. Three-phase skill (survey → ranked Tech/Design/CX suggestions → save to `briefs/YYYY-MM-DD.md`). Hard rule baked in: brief is suggestions only, never edits code. (6) **Generated and saved today's brief** at `CTO - TS/briefs/2026-05-07.md` as the first real test. Brief flagged T1–T5 for tech, D1 (no design items), C1–C3 for CX, and called out launch risk for RG as Medium. (7) Critical-gaps queue progress: gap #2 (tests) closed in session 35, gap #3 (staging) closed today. Remaining for RG: gap #1 (UptimeRobot, Mickey's task), gap #4 stage 2 (scoped admin tokens), gap #5 (DB restore verification). **Outstanding from today:** FRONTEND_URL on Railway staging still points to prod URL; CI workflow needs `staging` branch added to triggers; executor skills (`cx-build`, `design-build`, `tech-build`) still to be built next session. |
---
