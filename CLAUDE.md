...
> Last updated: 20 April 2026 (session 8). See "Session-end protocol" at the bottom of this file — follow it at the end of every session.
...
3. ~~Test Goalserve against live data~~ PARTIALLY DONE 20 Apr — API calls working (verified via timing tests), parallel fetch deployed. Still need to verify round mapping and player name matching once R1 matches are scheduled
4. **Set lock time overrides for R64+** — once order of play is announced, update `activeTournament.js` with actual first match times minus 1 hour (now also auto-set by `autoSetLockTimes()` in opsMonitor.js)
...
8. ~~Pre-Madrid: Mobile app sync~~ DONE 20 Apr — JWT auth migrated, legacy fallback removed from backend
...
12. ~~Build Phase 1 automation~~ DONE 19 Apr — `opsMonitor.js` + `routes/ops.js` + `ops_log` table + daily Cowork brief. Commit `5220fe4`.
...
16. **Clean up old headshot files** — `frontend/public/players/*.jpg` (173 individual files, 6.4MB) superseded by `player-sprite.webp` (205KB). Can be deleted to reduce repo size. Low priority.
