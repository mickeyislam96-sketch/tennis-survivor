...
> Last updated: 2 June 2026 (session 43 — RG QF-day fixes: (1) pick screen wrongly blocked players who share a FIRST name with a used pick — "Surname, Firstname" parsed wrong; fixed to full-name+ID match. (2) Reopened the QF window to 12:00 BST so Servena could re-pick. (3) R16/QF bracket was wrong — 3 pre-tournament R1 withdrawals never auto-resolved (no cancelled fixture) and the gap cascaded up; plus a 2-char surname "Wu" failed matching. Fixed via 1 seed-draw correction + 5 result overrides; bracket fully healed. Session 42 (1 Jun): staging-scraper-poisons-prod incident + non-picker leaderboard fix.) See "Session-end protocol" at the bottom of this file — follow it at the end of every session.
...
## Current tournament state (as of 2 June 2026)
...
### Roland Garros 2026 (ACTIVE — QF stage as of 2 Jun; entry closed)
- Tournament: Roland-Garros 2026 (Grand Slam, 128 draw, clay). **First Grand Slam this codebase has run.**
- Status: `active`. R1–R16 played. **QF window: lock was an 08:00Z estimate but QF matches don't start until 3 Jun; reopened to 2026-06-02 11:00 UTC (12:00 BST) so Servena could re-pick after the name-collision bug (config `lockTimeOverrides.QF`).**
- **QF (as of 2 Jun, after bracket repair):** Berrettini v Arnaldi · **Auger-Aliassime v Cobolli** · Mensik v Fonseca · **Jodar v Zverev**.
- **Live pool state (2 Jun 2026):** 5 entrants, **1 still in — Servena Williams (survived 4, sole survivor)**. Out: Casper (R32, non-picker), Mick (R64), Rafa (R1), Sabalenka (R1). No official winner yet (Option B: declared only at tournament completion).
- **Bracket repaired 2 Jun (commits `4adcc0d`,`69d2682`):** 3 pre-tournament R1 withdrawals (Fils→De Jong seed-draw fix; van Assche & Wu R1 overrides) + R64 Cobolli-d-Wu override (2-char surname). `manualResultOverrides` now has 5 entries. See `feedback_overlay_propagation_cascade.md`.
- **Per-round Railway updates as the tournament advances:** scraper `DEFAULT_ROUND` (currently `R1`, cosmetic), and `activeTournament.js` lock-time overrides. The QF→SF→F lock times in `lockTimeOverrides` are estimates until each order of play is announced.
...
20. **Rotate prod `ADMIN_SECRET` + give staging its own (security).** The staging scraper (and any cloned-env service) holds the **prod** admin secret. On 1 Jun its `BACKEND_URL` was pointed at prod and it corrupted production data for hours. `BACKEND_URL` is now fixed to staging, but the shared secret means a single env-var flip re-opens the hole. Rotate prod's `ADMIN_SECRET` (Railway backend + the **prod** scraper) and set a distinct secret on staging. See `feedback_staging_scraper_poisons_prod.md`.
21. **Audit every non-prod env's scraper `BACKEND_URL` after any environment clone.** A cloned scraper inherits prod's `BACKEND_URL`/`ADMIN_SECRET` and will silently write to prod. Make this a step in `project_staging_environment.md` and any future env-clone runbook. (Staging fixed 1 Jun; pause the staging scraper to save compute + remove the prod secret from a non-prod service.)
22. **Apply the mobile pick name-collision fix.** `mobile-pick-namebug-fix.patch` is in the `CTO - TS` workspace (same "Surname, Firstname" used-player bug as web `fcea805`). Couldn't push it — this session's token is scoped to the web repo only (403 on `tennis-survivor-mobile`). Apply with `git am mobile-pick-namebug-fix.patch` in the mobile repo, or add the mobile repo to the token scope and I'll push. Mobile is pre-App-Store so no users affected yet.
23. **Overlay follow-up: auto-heal pre-tournament withdrawal replacements with no cancelled fixture.** The auto-replacement only swaps when FlashScore shows a cancelled match; pre-event withdrawals don't, so the R1 match never resolves and the gap cascades up the bracket (2 Jun RG: Fils/Kypson/Giron slots). For now these need a manual seed-draw edit or `manualResultOverrides`. Riskier overlay change — own session, not mid-tournament. See `feedback_overlay_propagation_cascade.md`.
24. **Overlay follow-up: harden short-surname matching.** 2-char surnames (e.g. "Wu") fail `surnameSubsetMatch`, so a fixture can go unmatched even with both players present (2 Jun RG: R64 Cobolli vs Wu needed a manual override). See `feedback_overlay_propagation_cascade.md` / `feedback_double_barrel_surnames.md`.
...
| Date | Summary |
|---|---|
| 2 Jun 2026 (session 43) | **RG QF-day: pick name-collision fix, QF window reopen, full R16/QF bracket repair.** Three threads. **(1) Pick name collision (`fcea805`).** Mickey: "won't let Servena play Zverev, she hasn't used him." The pick screen's "already used" check did `name.split(' ').pop()` to get a surname, but names are `"Surname, Firstname"`, so it returned the FIRST name. Servena's used set was first names `{alexander,tommy,frances,rafael}`; **Zverev, Alexander** → "alexander" collided with **Blockx, Alexander** (R1) → Zverev's Pick button hidden. Backend would've accepted it (matches full name) — purely client-side. Fixed to match whole normalised name + ID (mirrors backend). Verified via harness (Zverev pickable, Jodar still blocked). **Same bug in mobile** (`PickScreen.tsx` + `DrawScreen.surname()`) — patch saved to workspace `mobile-pick-namebug-fix.patch`, NOT pushed (token scoped to web repo; 403 on mobile). New memory `feedback_picks_used_player_name_collision.md`. **(2) QF window reopen (`6bf8a3a`).** Mickey: reopen QF till 12:00 BST. The 08:00Z lock was an estimate; QF matches don't start until 3 Jun and none had started, and only Servena is alive, so reopening was clean (no hindsight). Set runtime lock-override (instant) + config `lockTimeOverrides.QF=2026-06-02T11:00:00Z` (durable). Verified config-driven after deploy (runtimeLockOverrides empty, QF isOpen). **(3) R16/QF bracket wrong (`4adcc0d`,`69d2682`).** Mickey: Felix Auger-Aliassime should be in; Tabilo & Svajda should be out. Scraper was CORRECT (AA d. Tabilo, Cobolli d. Svajda at R16) but the overlay wasn't applying it — bracket had null-player slots. Root: 3 R1 slots held pre-tournament withdrawals whose LL replacements never auto-resolved (the swap only fires on a "cancelled" fixture, which FlashScore doesn't show for pre-event withdrawals), so those R1 matches got no winner and the gap cascaded R64→R32→R16→QF. Fixes: seed-draw pos 120 Fils(s17, withdrew)→De Jong (LL, NED, won — wasn't in slot so needed the seed edit); R1 overrides van Assche-d-Kypson-slot (real LL Gaubas, `loserDisplayName`) and Wu-d-Giron-slot (scraper missing this R1 entirely); then R64 Cobolli-d-Wu override because the 2-char surname "Wu" failed `surnameSubsetMatch` even after propagating. `dataSource` healed 109→119 matched, overrides 2→5. **Verified:** 0 null/no-winner matches in played rounds; Tabilo & Svajda `roundEliminated=R16`; QF = AA v Cobolli / Jodar v Zverev / Berrettini v Arnaldi / Mensik v Fonseca; Servena's QF pool = correct 8 (Zverev present, Tabilo/Svajda gone); leaderboard unchanged (Servena alive, survived 4). New memory `feedback_overlay_propagation_cascade.md`. **Two overlay follow-ups flagged** (own session, not mid-tournament): auto-heal pre-event withdrawal replacements w/o cancelled fixture; harden short-surname matching. |
| 1 Jun 2026 (session 42) | **RG outage: a rogue STAGING scraper was poisoning production; plus a leaderboard non-picker bug.** Mickey reported (in effect) survived=0 for everyone, non-pickers not eliminated, leaderboard stuck on R16. **Diagnosis:** `/api/draw/bracket` returned pure `seed_draw` (0 completed) while `/api/health` adapter showed 95 fixtures — the asymmetry (overlay applies the 27-May wrong-event window guard, the adapter path does not) meant the cache held **wrong-event data**. `/api/admin/scraper-fixtures` confirmed it: 95 fixtures, Final "Sinner d. Ruud", dates 8–17 May = **Rome 2026**, not RG. The scraper run logs showed prod (`valiant-forgiveness`, prod env) correctly fetching french-open and posting 122 RG fixtures at :02 — but the cache held Rome at :04. **Root cause:** the **staging** env scraper (cloned from prod, session 36) still had `BACKEND_URL=…tennis-survivor-production…`, `FLASHSCORE_URL/RESULTS_URL=…/rome/…`, and the **prod** `ADMIN_SECRET`. It scraped Rome and POSTed to PROD every hour at :04, two minutes after the real RG scraper, clobbering it (`setScrapedResults` replaces the whole cache; last writer wins). **Fix (Railway via Chrome MCP):** repointed the staging scraper — `BACKEND_URL`→`tennis-survivor-staging…`, URLs→`french-open`; deployed; forced a prod Run-now. Cache healed to 122 RG fixtures; bracket `seed_draw+scraper(107)+overrides(2)`; QF pool 111→~8. **Two more fixes pushed to main:** (a) `c73523f` — recorded 2 R64 walkover winners in `manualResultOverrides` (Tabilo d. Vacherot, de Minaur d. Blockx; winners confirmed by their completed R32 fixtures), clearing `+REVIEW(2)`. (b) `16761e3` — `gradeMember()` in `leaderboard.js` now walks every LOCKED round so a non-picker (Casper: R1+R64 won, no R32 pick after R32 locked) is eliminated at R32 instead of shown alive; previously it graded only existing picks, so the leaderboard (2 alive) disagreed with `/api/pools` (1 alive). Verified live: leaderboard = 1 alive (Servena, survived 4), Casper ELIMINATED R32, matches `/api/pools`; bracket + QF pool correct; 0 walkover-pending. **Latent risk flagged:** the staging scraper still holds the prod `ADMIN_SECRET` — rotate prod secret + give staging its own to fully close the cross-env write hole. New memory `feedback_staging_scraper_poisons_prod.md`; updated non-picker + staging + walkover memories. Done ~35 min before QF opened (17:00 UTC). |
| 22 May 2026 (session 41) | **Pre-launch QA audit of Roland Garros + free-pool UI fixes.** Full audit before inviting players. Backend all clean (all-round deadlines sane, invite round-trips case-insensitively, non-members 403, empty/1-member leaderboard fine, R1 pool 111 players all with opponents, 128-draw bracket correct). Headshots 105/111 (6 low-ranked fall back to initials — fine). **Fixes (commits c733f33, 7c7fe6f, c0edba8; empty retrigger 83202f5):** (1) `JoinGroup.jsx` — the `betaFree` flag it branched on is **set nowhere** in the codebase, so free pools rendered 'Entry fee £0', a 'Join for £0 →' button + non-refundable disclaimer. Replaced with `isFree = entryFeeCents===0`: entry shows 'Free', prize-pool stat hidden, 'Join free →', free badge shown, disclaimer hidden, lede/rules drop the 'prize pot' promise. (2) `GroupHome.jsx` registered/active/complete Hero metas showed 'Prize pool £0'/'Entry fee £0' — prize-pool now hidden when 0, entry shows 'Free'. (3) `GroupHome.jsx` 'Down to one — final stretch' lede fired for a 1-member pre-start pool; guarded with `totalMembers > 1`. (4) FE registry surface 'Clay' → 'Clay (outdoor)' (matches BE); Homepage step-3 copy no longer hard-promises cash on a free launch. All verified live via Chrome. **Vercel gotcha:** the git webhook silently MISSED commit c733f33 (no deployment created) — an empty commit re-triggered it; after any push verify the Vercel deployment actually appears. **Mickey hands-off:** repaired the malformed git remote (token was glued after `.git`); confirmed clean. Mickey does NOT need to `git pull` for the product to work — Vercel/Railway deploy from GitHub and the agent pushes from /tmp clones; only pull to run code locally. New memory `feedback_free_pool_ui.md`. |
