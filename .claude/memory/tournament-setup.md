...
description: 16-step checklist for launching new tournaments. Covers config, seed draw, lock times, scraper mapping, headshots, and common gotchas. Reference doc in CTO - TS folder and pushed to repo.
type: reference
...
**4 phases, 16 steps:**
1. Before draw (config, DB group, Railway env, scraper name mapping)
...
**Common gotchas:** lock times not set (users pick after matches start), incomplete scraper name mapping (players not overlaying), stale ACTIVE_TOURNAMENT (needs Railway restart), qualifier name updates, draw size mismatch (96-draw has 128 positions).
