...
- [Pick used-player name collision](feedback_picks_used_player_name_collision.md) — "already used" must match full name + ID, never split(' ').pop() on "Surname, Firstname" (returns first name; Blockx/Zverev both → Alexander). Web fcea805; mobile patch in workspace.
- [Early-round breaks cascade up the bracket](feedback_overlay_propagation_cascade.md) — unresolved R1 match (pre-event withdrawal w/ no cancelled fixture, or 2-char surname) leaves null-player slots that climb R64→R32→R16→QF. Diagnose via lowest null/no-winner round; fix roots with seed-draw edit or manualResultOverrides. 2 overlay follow-ups noted.
